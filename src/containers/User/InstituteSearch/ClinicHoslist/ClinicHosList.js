...
import Classes from './ClinicHosList.module.css'
...
class HospClinicLists extends Component{
    constructor(props){
        super(props)
            this.state={
             hospClinic:[],
    ...
    componentDidMount(){
        ...
        if((lattitude === null) || (longitude === null) || (lattitude.length === 0) || (longitude.length === 0) ){
            ...
            .then(response=>{
                ...
                this.setState({hospClinic:response.data,pageCount: Math.ceil(data.length / this.state.perPage),
    ...
    loadMoreData() {
		const data = this.state.hospClinic;
    ...
    render(){
        ...
        if(this.state.loading){
            ...
            if(this.state.serach === ""){
                hospClinic = (this.state.orgHospital.length===0?<p className='NoResults'>No Institute Registerd</p>:this.state.orgHospital.map((item,index)=>{
                return(
            ...
            const filterd = this.state.hospClinic.filter((items)=>{
               return items = items.ISTNAME.toLowerCase().includes(this.state.serach.toLowerCase())
                })
                hospClinic = (filterd.length===0?<p className={Classes.NoResults}>No matches found</p>:filterd.map((item,index)=>{
               return(
                <Hospital instName={item.ISTNAME} 
                location={item.ADDRESS} phnno={item.MOBILE}
                instImage={item.COMPANY_DP} key={item.ISTID}
        ...
        <BookingLayout nearByClick={this.nearBtnClickHandler} loctnName={this.state.locName} >
            ...
            <div className={Classes.Search}>
                <input type='text' placeholder='Choose a hospital' className={Classes.SerachIcon} onChange={this.inputChangeHandler} value={this.state.serach}/>
            </div>
...
export default HospClinicLists