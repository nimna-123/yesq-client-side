...
class InstituteLists extends Component{
    constructor(props){
        super(props)
            this.state={
             institutes:[],
    ...
    componentDidMount(){
        ...
        const origin = 'NA'
        axios.post('/list/institute',{group:this.props.match.params.grp,category:this.props.match.params.catg,origin:origin})
        .then(response=>{
            var data = response.data;
            var slice = data.slice(this.state.offset,this.state.offset + this.state.perPage)
            this.setState({institutes:response.data,pageCount: Math.ceil(data.length / this.state.perPage),
            orgInstitutes:slice,loading:false})
          })
        .catch((err) =>{
 ...
 inputChangeHandler = (e) =>{
    ...
    instituteBookingHandler = (selectedInst) =>{
    ...
    render(){
        ...
        if(this.state.loading){
            ...
            institutes = (this.state.orgInstitutes.length===0?<p className='NoResults'>No Institute Registerd</p>:this.state.orgInstitutes.map((item,index)=>{            
                return(
                        <Institute instName={item.ISTNAME} 
                        location={item.ADDRESS} phnno={item.MOBILE}
                        instImage={item.COMPANY_DP} key={item.ISTID}
                        email={item.EMAIL} clickIns={()=>this.instituteBookingHandler(item)} 
            ...
            const filterd = this.state.institutes.filter((items)=>{
                return items = items.ISTNAME.toLowerCase().includes(this.state.serach.toLowerCase())
                 })
                 institutes = (filterd.length===0?<p className='NoResults'>No matches found</p>:filterd.map((item,index)=>{
                return(
                    <Institute instName={item.ISTNAME} 
                    location={item.ADDRESS} phnno={item.MOBILE}
                    instImage={item.COMPANY_DP} key={item.ISTID}
        ...
        return(
            <BookingLayout nearByClick={this.nearBtnClickHandler} loctnName={this.state.locName}>
               ...
               <div className={Classes.Search}>
                        <input type='text' placeholder='Search......' className={Classes.SerachIcon} onChange={this.inputChangeHandler} value={this.state.serach}/>
                </div>
                ...
                {this.state.orgInstitutes.length!==0? <ReactPaginate
                    previousLabel={"prev"}
                    nextLabel={"next"}
                    breakLabel={"..."}
                ...
                <div className={Classes.PaginationPad}>
